<h1>Normality Analysis</h1>
  <p>Use this dialog to assess the univariate and multivariate normality of a set of variables via formal tests and various plots.  It provides a number of test statistics, plots, and an outlier analysis.  Choose the variables whose distribution
  will be analyzed on the first tab. At least two variables must be specified.  Only variables with a scale measurement level can be analyzed,
  Choose the desired tests, plots, and summary statistics on the <i>Tests and Plots</i> tab, and choose the desired outlier analysis on the <i>Outliers</i> tab.  
  While there is no limit on the number of variables that can be analyzed, some plots may be hard to read with a large number of variables.</p>
  
  <p>It may be useful to use split files (<i>Data &gt;Split File</i>) in assessing normality for subgroups such as occur with ANOVA as a separate analyses is produced for each split.  If scaling, each split is scaled separately.  Split file variables cannot be included in the analysis list.</p>
  ...
  <p>Check the boxes for the desired plots.  The univariate plots are a boxplot of all the variables and a paired histogram and qqplot for each variable.  The multivariate plots are two variable scatterplots for each pair of variables.</p>
...
<h2>Outliers</h2>
<p>Check the <i>Show multivariate outliers</i> box to produce a table of outliers showing their Mahalanobis distance.  An ID variable must be specified for this output in order to identify the outlier points.
The id values must be unique across cases.  The id variable must not be included in the distribution list.<p>
...
<p>Many statistical tests, such as the Student's t-test and ANOVA assume
...
distribution, so the normality assumption is less important in large samples. </p>
...
<i>Example</i>:- If a retail company wants to analyze the annual sales figures of its stores to determine if the sales data is normally distributed, then the Cramér-von Mises test can be applied for the assessment.</p>
...
<p>Since each variable is plotted against all the other variables, there may be a lot of plots.  Enter a value, k, in the Number of Variables to Plot box to limit the
plots to the first k variables.  The plot is a SPLOM of all those variables.</p>
<p>The size of each plot in the SPLOM can be specified as the number of pixels in each dimension.  The default is 100 pixels.</p>
...
Gokmen Zararsiz.
  ...
  © Copyright Jon K Peck and IBM Corp. 2025</p>
